...
def des_encrypt_block(block8, subkeys):
    bits = bits_from_bytes(block8)
    bits = permute(bits, IP)
    L, R = bits[:32], bits[32:]
    for subkey in subkeys:
        L, R = R, xor(L, permute(sbox_substitute(xor(permute(R, E), subkey)), P))
    bits = permute(R + L, FP)
...
def des_encrypt(plaintext, key):
    ...
    subkeys = generate_subkeys(bits_from_bytes(key))
    plaintext = pad(plaintext)
    blocks = [plaintext[i:i + 8] for i in range(0, len(plaintext), 8)]
    return b''.join(des_encrypt_block(b, subkeys) for b in blocks)

def des_decrypt(ciphertext, key):
    key = key[:8].ljust(8, b'\0')
    subkeys = generate_subkeys(bits_from_bytes(key))
    blocks = [ciphertext[i:i + 8] for i in range(0, len(ciphertext), 8)]
    return unpad(b''.join(des_decrypt_block(b, subkeys) for b in blocks))

if __name__ == "__main__":
    print("DES - Encrypt/Decrypt")
    while True:
        print("\nChoose an option: [E]ncrypt, [D]ecrypt, [Q]uit")
        ...
        if choice == 'q':
            ...
            ciphertext_hex = input("Enter ciphertext: ").strip()
            key_str = input("Enter key: ")
